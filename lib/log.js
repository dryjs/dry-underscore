...
exports.mixin = function(_){
    ...
    function makeLogger(options){
        ...
        log.defaultLevel = function(){ return(log._defaultLevel); };
        log.levels = function(){ return(log._levels); };
        log.priorities = function(){ return(log._priorities); };
        log.priority = function(level){ 
            if(log._priorities[level]){
                return(log._priorities[level]);
            }else{
                return(-1); 
            }
        };

        log._defaultLevel = 'emerg';
        ...
        log._priority = { 'emerg' : 0, 'alert' : 1, 'crit' : 2, 'error' : 3, 'warning' : 4, 'notice' : 5, 'info' : 6, 'debug' : 7 };
        
        function consoleTransport(logLevel, timestamp, entry){ 
            var writer = _.stdout;
            var priority = _.indexOf(log.priorities(), logLevel);
