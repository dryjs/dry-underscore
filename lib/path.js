...
exports.mixin = function(_){
    ...
    lib.relative = nativePath.relative;
    lib.resolve = nativePath.resolve;
    ...
    lib.name = function(path){
        var file_name = this.file(path);
        return this.extension(file_name, "");
    };
    ...
    lib.extension = function(path, newExt){
        ...
        if(fileSplit.length === 1){ fileSplit.push(""); }

        var fileExt = fileSplit.pop() || "";

        if(newExt === undefined){ return(fileExt); }

        if(newExt){ fileSplit.push(newExt); }
    ...
    lib.fun = function(root, base){
        ...
        return(function(){
            ...
            return(lib.normalize(lib.join(path, lib.join.apply(null, arguments))));
        });
    };

    lib.home = lib.fun(function(){ return(process.env.HOME || process.env.HOMEPATH || process.env.USERPROFILE); });

    lib.cwd = lib.fun(process.cwd);
