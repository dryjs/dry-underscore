...
test("walk", function(){
    ...
    var obj = {
        obj: {
            a: [0, { one: 1 }, [2, 3]],
            o: { zero: 0, one: { one: 1 }, two: [2, 3] },
            trim: ["no", "bad"]
        },
        a: [0, { one: 1 }, [2, 3]],
    };

    var expected_keys = _.concat(
        ["obj"],
        ["a", 0, 1, "one", 2, 0, 1],
        ["o", "zero", "one", "one", "two", 0, 1],
        ["a", 0, 1, "one", 2, 0, 1]
    );

    var expected_vals = _.concat(
        [obj.obj],
        [obj.obj.a, 0, obj.obj.a[1], 1, obj.obj.a[2], 2, 3],
        [obj.obj.o, 0, obj.obj.o.one, 1, obj.obj.o.two, 2, 3],
        [obj.obj.a, 0, obj.obj.a[1], 1, obj.obj.a[2], 2, 3]
    );
    ...
    _.walk(obj, function(val, key){
        if(key === "trim"){ return(false); }
        vals.push(_.clone(val));
        keys.push(key);
    });

    eq(keys, expected_keys);
    eq(vals, expected_vals);
...
test("getters / setters", function(){
    ...
    function setup_hash_functions(o){
        o.get = _.getter("_foo", "_get");
        o.get_set = _.getter_setter("_foo", "_get_set");
        o.get_key = _.getter_key("_foo", "_get_key");
        o.get_set_key = _.getter_setter_key("_foo", "_get_set_key");
        o.get_set_key_locked = _.getter_setter_key("_foo", "_get_set_key_locked", true);
    };

    function setup_hash_object(o){
        o._foo = {};
        o._foo._get = "get";
        o._foo._get_set = "get_set";
        o._foo._get_key = [0,1,2];
        o._foo._get_set_key = [0,1,2];
        o._foo._get_set_key_locked = [0,1,2];
    }
    ...
    function test_hash_prototype(){ setup_hash_object(this); };
    setup_hash_functions(test_hash_prototype.prototype);

    var test_hash_object = {};
    setup_hash_object(test_hash_object);
    setup_hash_functions(test_hash_object);

    var test_hash_prototype_object = new test_hash_prototype();

    run_tests(test_hash_object);
    run_tests(test_hash_prototype_object);
...
test("moment-duration", function(){ eq(_.moment.duration(123, "minutes").format("h:mm:ss"), "2:03:00"); });
...
test('leaf', function(){
    eq(_.leaf({ 'A': 'b' }, 'A'), 'b');
    eq(_.leaf({ 'A': 'b' }, 'a.b'), undefined);
    eq(_.leaf({ 'A': 'b' }, 'a.b.c'), undefined);
    eq(_.leaf({}, 'a.b.c'), undefined);
    eq(_.leaf(null, 'a.b.c'), undefined);
    eq(_.leaf(['a', 'b'], 1), 'b');
    eq(_.leaf({ 'A': { 'b': { 'c' : 'c' } } }, 'c'), undefined);
    eq(_.leaf({ 'A': { 'b': { 'c' : 'c' } } }, 'A.b.c'), 'c');
    eq(_.leaf({ 'A': { 'b': { 'c' : 'c' } } }, 'A.b'), { 'c': 'c' });
    eq(_.leaf({ 'A': { 'b': { 'c' : 'c' } } }, 'g'), undefined);
});
...
test("replace", function(){
    eq(_.replace("a b a b", "a ", ""), "b b");
    eq(_.replace("a b a b", "a ", "", true), "b a b");
    eq(_.replace("/a /b a b", "/", "", false), "a b a b");
    eq(_.replace("a $b a b", "$", "", false), "a b a b");
});
...
function test_prevent_override(f){
    eq(f({ type: 'object' }), 'object');
    eq(f({ type: 'number' }), 'object');
    eq(f({ type: 'string' }), 'object');
    eq(f({ type: 'array' }), 'object');
    eq(f({ type: 'boolean' }), 'object');
    eq(f({ type: 'null' }), 'object');
    eq(f({ type: 'undefined' }), 'object');
}
...
test("dry_types", function(){
    test_prevent_override(_.dry_types);
...
function test_type_match(f){
    ...
    eq(true, f([t, x], ['object', 'array'], true));
    eq(true, f(undefined, ['object', 'array', 'undefined'], true));
    eq(true, f([t, x], 'array'));
